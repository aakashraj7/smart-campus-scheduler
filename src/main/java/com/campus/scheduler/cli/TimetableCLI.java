...
public class TimetableCLI {
    ...
    private void generateTimetables() {
        System.out.println("\n[INFO] Initializing Intelligent Timetable Generator...");
        ...
        if (result.isSuccess()) {
            System.out.println("\n+-------------------------------------------------------------------+");
            System.out.println("| [OK] SUCCESS! Timetables generated for " + result.getClassSchedules().size() + " classes!     |");
            System.out.println("+-------------------------------------------------------------------+");
            ...
            System.out.println("[OK] All timetables have been saved to MongoDB database!");
        } else {
    ...
    private void viewByClass(Scanner scanner) {
        ...
        if (timetables.isEmpty()) {
            System.out.println("\n[INFO] No timetables generated yet. Please run the Auto-Schedule engine first.");
            return;
        }

        System.out.print("\nEnter Class ID to view (e.g., IT-C, CSE-3A): ");
    ...
    private void viewByFaculty(Scanner scanner, User currentUser) {
        ...
        if ("FACULTY".equalsIgnoreCase(currentUser.getRole()) && currentUser.getFacultyId() != null) {
            ...
            System.out.println("\nViewing schedule for your Faculty ID: " + facultyId);
        } else {
    ...
    private void manualSwap(Scanner scanner) {
        AcademicConfig config = configDao.getConfig();
        System.out.print("\nEnter Class ID to swap periods in (e.g., IT-C, CSE-3A): ");
        String classId = scanner.nextLine().trim().toUpperCase();
        ...
        System.out.println("\n+-------------------------------------------------------------------+");
        System.out.println("|                MANUAL PERIOD SWAP FOR " + String.format("%-19s", classId) + " |");
        System.out.println("+-------------------------------------------------------------------+");
        ...
        if (slot1 == null && slot2 == null) {
            System.out.println("\n[ERROR] Both target slots are empty. Nothing to swap.");
            return;
        }
        ...
        timetableDao.saveOrUpdate(tt);
        System.out.println("\n[OK] Manual swap complete! Updated timetable saved to MongoDB.");
        System.out.println(TableFormatter.renderClassTimetable(classId, tt.getSlots(), config));
    }
