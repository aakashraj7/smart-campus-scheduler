...
public class AcademicSetupCLI {
    ...
    public void showSetupMenu(Scanner scanner) {
        while (true) {
            System.out.println("\n========================================================");
            System.out.println("            ACADEMIC SETUP & DATA ENTRY MENU");
            System.out.println("========================================================");
            System.out.println("1. Manage Faculties (Workload Limits & Details)");
            System.out.println("2. Manage Subjects");
            System.out.println("3. Configure Class Subject Requirements (e.g. 5 DBMS, 4 Web, 3 OS)");
    ...
    private void manageFaculties(Scanner scanner) {
        while (true) {
            System.out.println("\n--- FACULTY MANAGEMENT ---");
            System.out.println("1. Register / Add New Faculty (Unified Profile & Account Creation)");
            ...
            if (choice.equals("1")) {
                ...
                if (facultyDao.deleteByFacultyId(facultyId)) {
                    System.out.println("✅ Faculty " + facultyId + " deleted.");
                } else {
        ...
        while (true) {
            ...
            if (choice.equals("1")) {
                ...
                while (true) {
                    ...
                    System.out.println("❌ Invalid Subject Code! Must be 2-20 alphanumeric characters without spaces.");
                }
                ...
                System.out.println("✅ Subject '" + name + "' (" + code + ") saved successfully!");
            } else if (choice.equals("2")) {
                List<Subject> list = subjectDao.getAllSubjects();
                if (list.isEmpty()) {
                    System.out.println("ℹ️ No subjects added yet.");
                } else {
                    System.out.println("\n-------------------------------------------------------------");
                ...
                if (subjectDao.deleteByCode(code)) {
                    System.out.println("✅ Subject " + code + " deleted.");
                } else {
    ...
    private void manageClassRequirements(Scanner scanner) {
        while (true) {
            System.out.println("\n--- CLASS REQUIREMENTS CONFIGURATION ---");
            ...
            if (choice.equals("1")) {
                ...
                while (true) {
                    ...
                    System.out.println("❌ Invalid Class ID! Use alphanumeric characters (e.g., CSE-3A).");
                }
                ...
                while (true) {
                    ...
                    System.out.println("❌ Invalid Semester! Enter a number between 1 and 10.");
                }
                ...
                while (true) {
                    ...
                    if (!ValidationUtil.isValidCode(subCode)) {
                        System.out.println("❌ Invalid Subject Code format.");
                        continue;
                    }

                    Subject s = subjectDao.findByCode(subCode);
                    if (s == null) {
                        System.out.println("⚠️ Warning: Subject '" + subCode + "' not found in Database. Please make sure to add it under Subjects menu first.");
                    }
                    ...
                    if (f == null) {
                        System.out.println("⚠️ Warning: Faculty '" + facId + "' not found in Database. Please make sure to add them under Faculty menu first.");
                    }

                    int periods;
                    while (true) {
                        System.out.print("Enter Periods Needed Per Week (1 to 20, e.g. 5 for DBMS, 4 for WEB, 3 for OS): ");
                        String input = scanner.nextLine().trim();
                        if (ValidationUtil.isValidIntRange(input, 1, 20)) {
                            periods = Integer.parseInt(input);
                            break;
                        }
                        System.out.println("❌ Invalid period count! Enter a number between 1 and 20.");
                    }
                ...
                System.out.println("✅ Saved class requirements for Class " + classId);
            } else if (choice.equals("2")) {
                List<ClassGroup> list = classDao.getAllClasses();
                if (list.isEmpty()) {
                    System.out.println("ℹ️ No class requirements configured yet.");
                } else {
                ...
                if (classDao.deleteByClassId(classId)) {
                    System.out.println("✅ Class " + classId + " configuration deleted.");
                } else {
    ...
    private void configureAcademicDays(Scanner scanner) {
        ...
        int periods;
        while (true) {
            System.out.print("Enter number of periods per day (1 to 10, e.g. 5, 6, 7): ");
            String input = scanner.nextLine().trim();
            if (ValidationUtil.isValidIntRange(input, 1, 10)) {
                periods = Integer.parseInt(input);
                break;
            }
            System.out.println("❌ Invalid periods count! Must be between 1 and 10.");
        }
